// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using backend_agua.Infraestructure.Database;

#nullable disable

namespace backend_agua.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260330182902_SupportMultiIncidencia")]
    partial class SupportMultiIncidencia
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.4")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("backend_agua.Models.Cisterna", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("Litros")
                        .HasColumnType("integer");

                    b.Property<Guid>("ReporteSuministroId")
                        .HasColumnType("uuid");

                    b.Property<int>("Tipo")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ReporteSuministroId");

                    b.ToTable("Cisternas");
                });

            modelBuilder.Entity("backend_agua.Models.Comuna", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("LiderCedula")
                        .HasColumnType("text");

                    b.Property<string>("LiderNombre")
                        .HasColumnType("text");

                    b.Property<string>("LiderTlf")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("ParroquiaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ParroquiaId");

                    b.ToTable("Comunas");
                });

            modelBuilder.Entity("backend_agua.Models.Comunidad", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ComunaId")
                        .HasColumnType("uuid");

                    b.Property<string>("LiderCedula")
                        .HasColumnType("text");

                    b.Property<string>("LiderNombre")
                        .HasColumnType("text");

                    b.Property<string>("LiderTlf")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ComunaId");

                    b.ToTable("Comunidades");
                });

            modelBuilder.Entity("backend_agua.Models.Conflicto", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Explicacion")
                        .HasColumnType("text");

                    b.Property<Guid>("ReporteIncidenciaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ReporteIncidenciaId");

                    b.ToTable("Conflictos");
                });

            modelBuilder.Entity("backend_agua.Models.Fuga", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Lugar")
                        .HasColumnType("text");

                    b.Property<Guid>("ReporteIncidenciaId")
                        .HasColumnType("uuid");

                    b.Property<int?>("Tipo")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ReporteIncidenciaId");

                    b.ToTable("Fugas");
                });

            modelBuilder.Entity("backend_agua.Models.Municipio", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Municipios");
                });

            modelBuilder.Entity("backend_agua.Models.Parroquia", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("LiderCedula")
                        .HasColumnType("text");

                    b.Property<string>("LiderNombre")
                        .HasColumnType("text");

                    b.Property<string>("LiderTlf")
                        .HasColumnType("text");

                    b.Property<Guid>("MunicipioId")
                        .HasColumnType("uuid");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("MunicipioId");

                    b.ToTable("Parroquias");
                });

            modelBuilder.Entity("backend_agua.Models.PersonaAfectada", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Cedula")
                        .HasColumnType("text");

                    b.PrimitiveCollection<int[]>("Condiciones")
                        .IsRequired()
                        .HasColumnType("integer[]");

                    b.Property<int>("Edad")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("ReporteSaludId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ReporteSaludId");

                    b.ToTable("PersonasAfectadas");
                });

            modelBuilder.Entity("backend_agua.Models.Reporte", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ComunidadId")
                        .HasColumnType("uuid");

                    b.Property<int>("Estatus")
                        .HasColumnType("integer");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsLeido")
                        .HasColumnType("boolean");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ComunidadId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("Reportes");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteIncidencia", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ChoferCedula")
                        .HasColumnType("text");

                    b.Property<string>("ChoferNombreApellido")
                        .HasColumnType("text");

                    b.Property<Guid>("ReporteId")
                        .HasColumnType("uuid");

                    b.Property<bool>("TieneConflictos")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneFugas")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneTrancas")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneVentaIlegal")
                        .HasColumnType("boolean");

                    b.Property<string>("VehiculoColor")
                        .HasColumnType("text");

                    b.Property<string>("VehiculoMarcaModelo")
                        .HasColumnType("text");

                    b.Property<string>("VehiculoPlaca")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ReporteId")
                        .IsUnique();

                    b.ToTable("ReporteIncidencias");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteParticipacion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("DetalleAlcaldia")
                        .HasColumnType("text");

                    b.Property<string>("DetalleGobernacion")
                        .HasColumnType("text");

                    b.Property<string>("DetalleInstitucionNacional")
                        .HasColumnType("text");

                    b.Property<int?>("Partido")
                        .HasColumnType("integer");

                    b.Property<Guid>("ReporteId")
                        .HasColumnType("uuid");

                    b.Property<bool>("TieneAlcaldia")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneGobernacion")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneInstitucionNacional")
                        .HasColumnType("boolean");

                    b.Property<bool>("TienePartido")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("ReporteId")
                        .IsUnique();

                    b.ToTable("ReporteParticipaciones");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSalud", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("CantidadCasosDiarrea")
                        .HasColumnType("integer");

                    b.Property<int>("CantidadCasosDolorAbdominal")
                        .HasColumnType("integer");

                    b.Property<int>("CantidadCasosVomitos")
                        .HasColumnType("integer");

                    b.Property<Guid>("ReporteId")
                        .HasColumnType("uuid");

                    b.Property<bool>("TieneDiarrea")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneDolorAbdominal")
                        .HasColumnType("boolean");

                    b.Property<bool>("TieneVomitos")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("ReporteId")
                        .IsUnique();

                    b.ToTable("ReporteSalud");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSuministro", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int?>("ApoyoAdicionalLitros")
                        .HasColumnType("integer");

                    b.Property<int?>("Caudal")
                        .HasColumnType("integer");

                    b.Property<int>("FamiliasBeneficiadas")
                        .HasColumnType("integer");

                    b.Property<TimeSpan?>("HorasSuministro")
                        .HasColumnType("interval");

                    b.Property<bool>("LlegaPorTuberia")
                        .HasColumnType("boolean");

                    b.Property<bool>("RecibeCisterna")
                        .HasColumnType("boolean");

                    b.Property<Guid>("ReporteId")
                        .HasColumnType("uuid");

                    b.Property<bool>("TieneTanque")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("ReporteId")
                        .IsUnique();

                    b.ToTable("ReporteSuministros");
                });

            modelBuilder.Entity("backend_agua.Models.Tanque", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ReporteSuministroId")
                        .HasColumnType("uuid");

                    b.Property<int>("Tipo")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ReporteSuministroId");

                    b.ToTable("Tanques");
                });

            modelBuilder.Entity("backend_agua.Models.Tranca", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<TimeSpan?>("Duracion")
                        .HasColumnType("interval");

                    b.Property<string>("Lugar")
                        .HasColumnType("text");

                    b.Property<string>("PropiciaNombre")
                        .HasColumnType("text");

                    b.Property<Guid>("ReporteIncidenciaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ReporteIncidenciaId");

                    b.ToTable("Trancas");
                });

            modelBuilder.Entity("backend_agua.Models.Usuario", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Cedula")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("ComunaId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ComunidadId")
                        .HasColumnType("uuid");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("ParroquiaId")
                        .HasColumnType("uuid");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Rol")
                        .HasColumnType("integer");

                    b.Property<int>("Status")
                        .HasColumnType("integer");

                    b.Property<string>("Telefono")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ComunaId");

                    b.HasIndex("ComunidadId");

                    b.HasIndex("ParroquiaId");

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("backend_agua.Models.Cisterna", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteSuministro", "ReporteSuministro")
                        .WithMany("Cisternas")
                        .HasForeignKey("ReporteSuministroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteSuministro");
                });

            modelBuilder.Entity("backend_agua.Models.Comuna", b =>
                {
                    b.HasOne("backend_agua.Models.Parroquia", "Parroquia")
                        .WithMany("Comunas")
                        .HasForeignKey("ParroquiaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Parroquia");
                });

            modelBuilder.Entity("backend_agua.Models.Comunidad", b =>
                {
                    b.HasOne("backend_agua.Models.Comuna", "Comuna")
                        .WithMany("Comunidades")
                        .HasForeignKey("ComunaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Comuna");
                });

            modelBuilder.Entity("backend_agua.Models.Conflicto", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteIncidencia", "ReporteIncidencia")
                        .WithMany("Conflictos")
                        .HasForeignKey("ReporteIncidenciaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteIncidencia");
                });

            modelBuilder.Entity("backend_agua.Models.Fuga", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteIncidencia", "ReporteIncidencia")
                        .WithMany("Fugas")
                        .HasForeignKey("ReporteIncidenciaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteIncidencia");
                });

            modelBuilder.Entity("backend_agua.Models.Parroquia", b =>
                {
                    b.HasOne("backend_agua.Models.Municipio", "Municipio")
                        .WithMany("Parroquias")
                        .HasForeignKey("MunicipioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Municipio");
                });

            modelBuilder.Entity("backend_agua.Models.PersonaAfectada", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteSalud", "ReporteSalud")
                        .WithMany("PersonasAfectadas")
                        .HasForeignKey("ReporteSaludId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteSalud");
                });

            modelBuilder.Entity("backend_agua.Models.Reporte", b =>
                {
                    b.HasOne("backend_agua.Models.Comunidad", "Comunidad")
                        .WithMany()
                        .HasForeignKey("ComunidadId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("backend_agua.Models.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Comunidad");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteIncidencia", b =>
                {
                    b.HasOne("backend_agua.Models.Reporte", "Reporte")
                        .WithOne("Incidencia")
                        .HasForeignKey("backend_agua.Models.ReporteIncidencia", "ReporteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Reporte");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteParticipacion", b =>
                {
                    b.HasOne("backend_agua.Models.Reporte", "Reporte")
                        .WithOne("Participacion")
                        .HasForeignKey("backend_agua.Models.ReporteParticipacion", "ReporteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Reporte");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSalud", b =>
                {
                    b.HasOne("backend_agua.Models.Reporte", "Reporte")
                        .WithOne("Salud")
                        .HasForeignKey("backend_agua.Models.ReporteSalud", "ReporteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Reporte");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSuministro", b =>
                {
                    b.HasOne("backend_agua.Models.Reporte", "Reporte")
                        .WithOne("Suministro")
                        .HasForeignKey("backend_agua.Models.ReporteSuministro", "ReporteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Reporte");
                });

            modelBuilder.Entity("backend_agua.Models.Tanque", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteSuministro", "ReporteSuministro")
                        .WithMany("Tanques")
                        .HasForeignKey("ReporteSuministroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteSuministro");
                });

            modelBuilder.Entity("backend_agua.Models.Tranca", b =>
                {
                    b.HasOne("backend_agua.Models.ReporteIncidencia", "ReporteIncidencia")
                        .WithMany("Trancas")
                        .HasForeignKey("ReporteIncidenciaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ReporteIncidencia");
                });

            modelBuilder.Entity("backend_agua.Models.Usuario", b =>
                {
                    b.HasOne("backend_agua.Models.Comuna", "Comuna")
                        .WithMany()
                        .HasForeignKey("ComunaId");

                    b.HasOne("backend_agua.Models.Comunidad", "Comunidad")
                        .WithMany("Usuarios")
                        .HasForeignKey("ComunidadId");

                    b.HasOne("backend_agua.Models.Parroquia", "Parroquia")
                        .WithMany()
                        .HasForeignKey("ParroquiaId");

                    b.Navigation("Comuna");

                    b.Navigation("Comunidad");

                    b.Navigation("Parroquia");
                });

            modelBuilder.Entity("backend_agua.Models.Comuna", b =>
                {
                    b.Navigation("Comunidades");
                });

            modelBuilder.Entity("backend_agua.Models.Comunidad", b =>
                {
                    b.Navigation("Usuarios");
                });

            modelBuilder.Entity("backend_agua.Models.Municipio", b =>
                {
                    b.Navigation("Parroquias");
                });

            modelBuilder.Entity("backend_agua.Models.Parroquia", b =>
                {
                    b.Navigation("Comunas");
                });

            modelBuilder.Entity("backend_agua.Models.Reporte", b =>
                {
                    b.Navigation("Incidencia")
                        .IsRequired();

                    b.Navigation("Participacion")
                        .IsRequired();

                    b.Navigation("Salud")
                        .IsRequired();

                    b.Navigation("Suministro")
                        .IsRequired();
                });

            modelBuilder.Entity("backend_agua.Models.ReporteIncidencia", b =>
                {
                    b.Navigation("Conflictos");

                    b.Navigation("Fugas");

                    b.Navigation("Trancas");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSalud", b =>
                {
                    b.Navigation("PersonasAfectadas");
                });

            modelBuilder.Entity("backend_agua.Models.ReporteSuministro", b =>
                {
                    b.Navigation("Cisternas");

                    b.Navigation("Tanques");
                });
#pragma warning restore 612, 618
        }
    }
}
